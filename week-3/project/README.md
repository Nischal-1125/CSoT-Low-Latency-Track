...
| Version | Runtime (ns) | Throughput (M ticks/s) |
|----------|------------|------------------------|
| Initial multithreaded implementation | 338,089,453 | 591.56 |
| Core-pinned version | 325,217,458 | 614.97 |
| Final submission | 294,771,087 | 678.49 |

### Key Optimizations

#### 1. Per-thread Partial Aggregation

Each worker thread accumulates statistics into its own private aggregation table:

- Eliminates synchronization on the hot path
- No locks required during aggregation
- Reduction happens only once at the end

#### 2. False Sharing Avoidance

Partial tables are aligned to cache-line boundaries:
...
| Prefetch Distance | Throughput (M ticks/s) |
| ----------------- | ---------------------- |
| 16                | ~591                   |
| 32                | Improved               |
| 64                | Improved               |
